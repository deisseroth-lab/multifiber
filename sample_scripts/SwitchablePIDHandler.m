classdef SwitchablePIDHandler < handle
    ...
    methods
        function obj = PIDHandler()
            ...
            device = devices(1);
            ao_ch = s.addAnalogOutputChannel(device.ID, 1, 'Voltage');
            s.Rate = 1000;
            s.IsContinuous = true;
            s.addlistener('DataRequired', @obj.provide);
            
            % Set up the analog input (switch)
            ai_ch = s.addAnalogInputChannel(device.ID, 6, 'Voltage');
            s.addlistener('DataAvailable', @(src, event) check_sync(src, event));
        ...
        function check_sync(obj, src, event)
            val = evt.Data(1);
            if val > 2.5
                obj.enabled = true;
            else
